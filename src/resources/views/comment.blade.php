        ...
        <div class="row">
            ...
            <div class="item__detail col-md-6 mt-5">
                            ...
                            <div class="comment__user-name left">
                                <div class="comment__user-icon-background">
                                    <div class="comment__user-icon">
                                        <img src="{{ $comment->user_url_thumbnail ? $comment->user_url_thumbnail : asset('images/user_no-name.jpeg') }}"
                                            alt="user_thumbnail">
                                    </div>
                                </div>
                ...
                <form class="comment__form" action="{{ route('comment.store', ['item_id' => $item->id]) }}" method="POST">
                    @csrf
                    <h2 class="comment__form-title">商品へのコメント</h2>
                    <textarea class="comment__form-content form-control" name="comment">{{ old('comment') }}</textarea>
                    @error('comment')
